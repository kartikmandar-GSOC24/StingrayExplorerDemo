...
class WarningHandler:
    ...
    def warn(self, message, category=None, filename=None, lineno=None, file=None, line=None):
...
async def load_event_data(event, file_selector, filename_input, format_input, output, warning_output):
    if not file_selector.value:
        output.value = "No file selected. Please select a file to upload."
        return

    file_paths = file_selector.value
    filenames = [name.strip() for name in filename_input.value.split(",")] if filename_input.value else []
    formats = [fmt.strip() for fmt in format_input.value.split(",")] if format_input.value else []

    if len(filenames) < len(file_paths):
        filenames.extend([f"default{i+1}" for i in range(len(filenames), len(file_paths))])
    ...
    try:
        ...
        for file_path, file_name, file_format in zip(file_paths, filenames, formats):
            event_list = await loop.run_in_executor(None, EventList.read, file_path, file_format)
            loaded_files.append(f"File '{file_path}' loaded successfully as '{file_name}' with format '{file_format}'.")
...
def create_data_ingestion_tab1():
    ...
    filename_input = pn.widgets.TextInput(
        name="Enter File Names", placeholder="Enter file names, comma-separated"
    )
    ...
    def on_click(event):
        ...
        asyncio.create_task(
            load_event_data(event, file_selector, filename_input, format_input, output, warning_output)
        )
